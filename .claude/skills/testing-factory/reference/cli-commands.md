...
| `--computes` | flag | Show only computes section |
...
Syncs DVT development environment (adapters, PySpark, JDBC, connectors).
...
- With version commented out in computes.yml -> installs default 3.5.8
- With explicit version in computes.yml -> installs that version
...
- `dvt run --target dbx_dev` -> override target
