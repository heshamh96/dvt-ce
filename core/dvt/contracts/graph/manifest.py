...
from dvt.adapters.exceptions import (
...
def _get_locality(
    ...
    elif macro.package_name in internal_packages or macro.package_name == "dvt":
    ...
    def find_generate_macro_by_name(
        ...
        - if package is not provided:
            - if there is a `generate_{component}_name` macro in the root
              project, return it
            - return the `generate_{component}_name` macro from the 'dvt'
              internal project
...
class Manifest(MacroMethods, dbtClassMixin):
    ...
    def build_flat_graph(self):
        ...
        self.flat_graph = {
            ...
            "functions": {
                k: v.to_dict(omit_none=False) for k, v in self.functions.items()
            },
    ...
    def _get_parent_adapter_types(self, adapter_type: str) -> List[str]:
        ...
        from dvt.adapters.factory import get_adapter_type_names
    ...
    # Called in dvt.compilation.Linker.write_graph and
    # dvt.graph.queue.get and ._include_in_cost
    ...
    # Called by dvt.parser.manifest._process_refs & ManifestLoader.check_for_model_deprecations
    def resolve_ref(
        self,
    ...
    # Called by dvt.parser.manifest._resolve_sources_for_exposure
    # and dvt.parser.manifest._process_source_for_node
    def resolve_source(
        self,
