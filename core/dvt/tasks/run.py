...
class DvtRunTask(RunTask):
    ...
    Before execution:
    1. Reads profiles.yml to get all output configs
    2. Resolves execution paths for all models (default/non-default pushdown, sling direct, duckdb compute)

    During execution:
    3. Returns DvtModelRunner for extraction models, stock ModelRunner for pushdown
    4. DvtModelRunner handles Sling/DuckDB orchestration transparently
    """
    ...
    def _resolve_models(self) -> None:
        ...
        try:
            ...
            logger.warning(f"DVT: Could not read profiles.yml for resolution: {e}")
            self._raw_profiles = {}
