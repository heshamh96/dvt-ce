...
clean_project_files_only = _create_option_and_track_env_var(
    ...
    help="If disabled, dvt clean will delete all paths specified in clean-paths, even if they're outside the dvt project.",
    default=True,
)
...
compile_docs = _create_option_and_track_env_var(
    "--compile/--no-compile",
    envvar=None,
    help="Whether or not to run 'dvt compile' as part of docs generation",
    default=True,
)
...
config_dir = _create_option_and_track_env_var(
    "--config-dir",
    envvar=None,
    help="Print a system-specific command to access the directory that the current dvt project is searching for a profiles.yml. Then, exit. This flag renders other debug step flags no-ops.",
    is_flag=True,
)
...
debug = _create_option_and_track_env_var(
    ...
    "-d/ ",
    envvar="DBT_DEBUG",
    help="Display debug logging during dvt execution. Useful for debugging and making bug reports.",
)
...
empty_catalog = _create_option_and_track_env_var(
    "--empty-catalog",
    help="If specified, generate empty catalog.json file during the `dvt docs generate` command.",
    default=False,
    is_flag=True,
)
...
event_time_end = _create_option_and_track_env_var(
    ...
    help="If specified, the end datetime dvt uses to filter microbatch model inputs (exclusive).",
    type=click.DateTime(),
    default=None,
)
...
event_time_start = _create_option_and_track_env_var(
    ...
    help="If specified, the start datetime dvt uses to filter microbatch model inputs (inclusive).",
    type=click.DateTime(),
    default=None,
)
...
exclude_resource_type = _create_option_and_track_env_var(
    ...
    help="Specify the types of resources that dvt will exclude",
    type=ChoiceTuple(
        [
            "metric",
            "semantic_model",
...
full_refresh = _create_option_and_track_env_var(
    ...
    help="If specified, dvt will drop incremental models and fully-recalculate the incremental table from the model definition.",
    is_flag=True,
)
...
host = _create_option_and_track_env_var(
    "--host",
    envvar="DBT_HOST",
    help="host to serve dvt docs on",
    type=click.STRING,
    default="127.0.0.1",
)
...
inline = _create_option_and_track_env_var(
    "--inline",
    envvar=None,
    help="Pass SQL inline to dvt compile and show",
)

inline_direct = _create_option_and_track_env_var(
    "--inline-direct",
    envvar=None,
    help="Internal flag to pass SQL inline to dvt show. Do not load the entire project or apply templating.",
    hidden=True,
)
...
log_file_max_bytes = _create_option_and_track_env_var(
    ...
    help="Configure the max file size in bytes for a single dvt.log file, before rolling over. 0 means no limit.",
...
record_timing_info = _create_option_and_track_env_var(
    ...
    "-r",
    envvar=None,
    help="When this option is passed, dvt will output low-level timing stats to the specified file. Example: `--record-timing-info output.profile`",
...
resource_type = _create_option_and_track_env_var(
    ...
    help="Restricts the types of resources that dvt will include",
    type=ChoiceTuple(
        [
            "metric",
            "semantic_model",
...
send_anonymous_usage_stats = _create_option_and_track_env_var(
    ...
    help="Send anonymous usage stats to dvt Labs.",
    default=True,
)
...
show_limit = _create_option_and_track_env_var(
    "--limit",
    envvar=None,
    help="Limit the number of results returned by dvt show",
    type=click.INT,
    default=5,
)
...
show_output_format = _create_option_and_track_env_var(
    "--output",
    envvar=None,
    help="Output format for dvt compile and dvt show",
...
warn_error = _create_option_and_track_env_var(
    ...
    help="If dvt would normally warn, instead raise an exception. Examples include --select that selects nothing, deprecations, configurations with no associated models, invalid test configurations, and missing sources/refs in tests.",
    default=None,
    is_flag=True,
)
...
warn_error_options = _create_option_and_track_env_var(
    ...
    default="{}",
    help="""If dvt would normally warn, instead raise an exception based on error/warn configuration. Examples include --select that selects nothing, deprecations, configurations with no associated models, invalid test configurations,
...
upload_artifacts = _create_option_and_track_env_var(
    ...
    help="Whether or not to upload the artifacts to the dvt Cloud API",
    default=False,
)
