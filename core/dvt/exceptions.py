...
class ContractBreakingChangeError(DbtRuntimeError):
    ...
    def message(self):
        ...
        return (
            "While comparing to previous project state, dvt detected a breaking change to an enforced contract."
            f"\n  - {reasons}\n"
...
class NoAdaptersAvailableError(DbtRuntimeError):
    ...
    def get_message(self) -> str:
        msg = (
            "No database adapters are installed.\n\n"
            "To fix this:\n"
            "  1. Edit ~/.dvt/profiles.yml and configure your database connection\n"
            "     (see https://docs.getdbt.com/docs/available-adapters for supported adapters)\n"
            "  2. Run 'dvt sync' to automatically install the required adapter\n"
        )
        return msg
...
class RefBadContextError(CompilationError):
    ...
    def get_message(self) -> str:
        ...
        msg = f"""dvt was unable to infer all dependencies for the model "{model_name}".
...
class DuplicateMacroNameError(CompilationError):
    ...
    def get_message(self) -> str:
        ...
        msg = (
            f'dvt found two macros with the name "{duped_name}" in the namespace "{self.namespace}"{extra}. '
            "Since these macros have the same name and exist in the same "
            "namespace, dvt will be unable to decide which to call. To fix this, "
...
class PythonLiteralEvalError(ParsingError):
    ...
    def get_message(self) -> str:
        msg = (
            f"Error when trying to literal_eval an arg to dvt.ref(), dvt.source(), dvt.config() or dvt.config.get() \n{self.exc}\n"
            "https://docs.python.org/3/library/ast.html#ast.literal_eval\n"
            "In dvt python model, `dvt.ref`, `dvt.source`, `dvt.config`, `dvt.config.get` function args only support Python literal structures"
        )
...
class DuplicateSourcePatchNameError(CompilationError):
    ...
    def get_message(self) -> str:
        ...
        msg = (
            f"dvt found two schema.yml entries for the same source named "
...
class DuplicateMacroPatchNameError(CompilationError):
    ...
    def get_message(self) -> str:
        ...
        msg = (
            f"dvt found two schema.yml entries for the same macro in package "
...
class NonUniquePackageNameError(CompilationError):
    ...
    def get_message(self) -> str:
        msg = (
            "dvt found more than one package with the name "
...
class PatchTargetNotFoundError(CompilationError):
    ...
    def get_message(self) -> str:
        ...
        msg = f"dvt could not find models for the following patches:\n\t{patch_list}"
        return msg
...
class AmbiguousAliasError(CompilationError):
    ...
    def get_message(self) -> str:
        ...
        msg = (
            f'dvt found two resources with the database representation "{self.duped_name}".\ndbt '
...
class AmbiguousResourceNameRefError(CompilationError):
    ...
    def get_message(self) -> str:
        ...
        msg = (
            f"When referencing '{self.duped_name}', dvt found nodes in multiple packages: {formatted_unique_ids}"
...
class AmbiguousCatalogMatchError(CompilationError):
    ...
    def get_message(self) -> str:
        msg = (
            "dvt found two relations in your warehouse with similar database identifiers. "
            "dvt\nis unable to determine which of these relations was created by the model "
            f'"{self.unique_id}".\nIn order for dvt to correctly generate the catalog, one '
...
class DependencyNotFoundError(CompilationError):
    ...
    def get_message(self) -> str:
        msg = (
            ...
            "Hint: You may need to run `dvt deps`."
        )
...
class DuplicatePatchPathError(CompilationError):
    ...
    def get_message(self) -> str:
        ...
        msg = (
            f"dvt found two schema.yml entries for the same resource named "
...
class DuplicateResourceNameError(CompilationError):
    ...
    def get_message(self) -> str:
        ...
        msg = f"""
dvt found two {pluralized} with the name "{duped_name}".

Since these resources have the same name, dvt will be unable to find the correct resource
...
class DuplicateVersionedUnversionedError(ParsingError):
    ...
    def get_message(self) -> str:
        msg = f"""
dvt found versioned and unversioned models with the name "{self.versioned_node.name}".
